...
from config import BASE_DIR, DATA_DIR, DOWNLOADS_DIR, LOCAL_MANGA_DIR
...
class LibraryManager:
    ...
    def run_background_maintenance(self) -> bool:
        ...
        try:
            for repair in (
                self.repair_missing_download_records,
                self.repair_missing_metadata,
                self.repair_missing_covers,
            ):
                try:
                    repair()
                ...
                with db.get_connection() as conn:
                    ...
                    if cur.fetchone()[0] == 0:
                        ...
                        for manga_id, manga in mangas.items():
                            ...
                            """, (
                                manga.get("id", manga_id),
                                manga.get("title", ""),
                                manga.get("description", ""),
                                manga.get("cover_url", ""),
                                manga.get("cover_path", ""),
                                manga.get("status", "unknown"),
                                manga.get("folder_name", ""),
                                tags,
                                manga.get("year", 0) or 0,
                                manga.get("last_read_chapter", ""),
                                manga.get("last_read_page", 0),
                                manga.get("last_read_chapter_num", ""),
                                manga.get("last_read_chapter_title", ""),
                                manga.get("last_read_source_id", ""),
                                manga.get("last_read_language", "tr"),
                                bool(manga.get("last_read_online", True)),
                                manga.get("last_read_at", 0)
                            ))
    ...
    def import_mal_entry(self, manga: Dict[str, Any], mal_entry: Dict[str, Any]) -> Dict[str, Any]:
        ...
        status_map = {
            ...
            "on_hold": "on_hold",
            "dropped": "dropped",
            "plan_to_read": "on_hold",
        }
        library_status = status_map.get(mal_status, "on_hold")
        self.add_manga(
            manga_id,
            str(manga.get("title") or mal_entry.get("title") or "Bilinmeyen Manga"),
            str(manga.get("description") or ""),
            str(manga.get("cover_url") or mal_entry.get("cover_url") or ""),
            str(manga.get("status") or "ongoing"),
        )
        with db.get_connection() as conn:
            ...
            """, (
                mal_id,
                mal_status,
                max(0, int(mal_entry.get("num_chapters_read") or 0)),
                max(0, int(mal_entry.get("num_volumes_read") or 0)),
                library_status,
                max(0, min(10, int(mal_entry.get("score") or 0))),
                json.dumps(self._clean_collections(collections), ensure_ascii=False),
                json.dumps(manga.get("tags") or [], ensure_ascii=False),
                int(manga.get("year") or 0),
                int(time.time()),
                manga_id,
            ))
            conn.commit()
    ...
    def add_downloaded_chapter(
        self, manga_id: str, chapter_id: str, chapter_num: str,
        title: str, language: str, page_filenames: list, chapter_dir: str | None = None
    ) -> Dict[str, Any]:
        with db.get_connection() as conn:
            cur = conn.execute("SELECT id FROM mangas WHERE id = ?", (manga_id,))
            if not cur.fetchone():
    ...
    def add_local_manga(
        ...
        *,
        ...
        # Mutlak yönetilen yollar Reader ve Full farklı kurulum klasörlerinde
        # çalıştırıldığında da aynı kullanıcı verisini göstermeye devam eder.
        ...
        with db.get_connection() as conn:
            conn.execute("""
                INSERT INTO mangas (
                    ...
                    'tr', 0, 0, 'reading', ?, ?, ?)
            """, (
                manga_id, title, stored_cover, folder_name,
                json.dumps(known_chapters), len(known_chapters), updated_at,
            ))
            for index, chapter in enumerate(chapters, start=1):
                ...
                """, (
                    str(chapter["id"]), manga_id, str(index),
                    str(chapter.get("title") or f"Bölüm {index}"),
                    json.dumps(list(chapter["pages"]), ensure_ascii=False), stored_path,
                ))
            conn.commit()
        return self.get_manga(manga_id) or {}
    ...
    def update_progress(
        ...
        *, manga_title: str = "", description: str = "", cover_url: str = "",
        status: str = "ongoing", chapter_num: str = "", chapter_title: str = "",
        source_id: str = "", language: str = "tr", online: bool = True,
        page_offset: float = 0.0, chapter_percent: float = 0.0,
    ) -> Dict[str, Any]:
        ...
        with db.get_connection() as conn:
            ...
            if not cur.fetchone():
                ...
                """, (manga_id, manga_title or "Bilinmeyen Manga", description, cover_url, status or "ongoing",
                      chapter_id, max(0, int(page_index)), chapter_num, chapter_title, source_id, language,
                      bool(online), at_time, page_offset, chapter_percent, at_time))
            else:
                updates = []
                params = []
                if manga_title: updates.append("title = ?"); params.append(manga_title)
                if description: updates.append("description = ?"); params.append(description)
                if cover_url: updates.append("cover_url = ?"); params.append(cover_url)
                if status: updates.append("status = ?"); params.append(status)
                
                updates.extend([
                    "last_read_chapter = ?", "last_read_page = ?", "last_read_chapter_num = ?",
                    "last_read_chapter_title = ?", "last_read_source_id = ?", "last_read_language = ?",
                    "last_read_online = ?", "last_read_at = ?"
                ])
                updates.extend(["last_read_offset = ?", "last_read_percent = ?"])
                updates.append("updated_at = ?")
                params.extend([
                    chapter_id, max(0, int(page_index)), chapter_num, chapter_title,
                    source_id, language, bool(online), at_time, page_offset, chapter_percent, at_time,
                ])
    ...
    def get_progress(self, manga_id: str) -> Dict[str, Any]:
        with db.get_connection() as conn:
            cur = conn.execute(
                "SELECT last_read_chapter, last_read_page, last_read_offset, last_read_percent FROM mangas WHERE id = ?",
                (manga_id,),
            )
            row = cur.fetchone()
            if row:
                return {
                    ...
                    "last_read_offset": row["last_read_offset"] or 0,
                    "last_read_percent": row["last_read_percent"] or 0,
                }
            return {"last_read_chapter": "", "last_read_page": 0, "last_read_offset": 0, "last_read_percent": 0}

    def _row_to_dict(
        self,
        manga_row: sqlite3.Row,
        chapters_rows: List[sqlite3.Row],
        *,
    ...
    def _normalized_chapter_numbers(cls, values: list) -> list[str]:
        numbers = {
            number for number in (cls._chapter_number(value) for value in values or [])
    ...
    def _unread_count(cls, known_chapters: list, last_read_chapter_num: Any) -> int:
        ...
        return sum(
            cls._chapter_number(chapter) > last_read
            for chapter in cls._normalized_chapter_numbers(known_chapters)
        )

    def _refresh_unread_count(self, manga_id: str, chapter_num: Any) -> None:
        with db.get_connection() as conn:
            row = conn.execute(
                "SELECT known_chapters FROM mangas WHERE id = ?",
                (manga_id,),
            ).fetchone()
            if not row:
                return
            ...
            conn.execute(
                "UPDATE mangas SET unread_count = ? WHERE id = ?",
                (self._unread_count(known, chapter_num), manga_id),
            )
            conn.commit()
    ...
    def update_library_metadata(
        ...
        *,
        ...
        with db.get_connection() as conn:
            ...
            """, (
                status, rating, note,
                json.dumps(cleaned_collections, ensure_ascii=False),
    ...
    def bulk_update_library(
        ...
        *,
        ...
        with db.get_connection() as conn:
            for manga_id in ids:
                row = conn.execute(
                    "SELECT collections FROM mangas WHERE id = ?",
                    (manga_id,),
                ).fetchone()
                if not row:
                    continue
                ...
                if collection:
                    ...
                    params.append(json.dumps(
                        self._clean_collections([*existing, collection[0]]),
                        ensure_ascii=False,
                    ))
                params.append(manga_id)
                conn.execute(
                    f"UPDATE mangas SET {', '.join(updates)} WHERE id = ?",
                    params,
                )
    ...
    def update_known_chapters(self, manga_id: str, chapter_numbers: list) -> Dict[str, Any] | None:
        with db.get_connection() as conn:
            row = conn.execute(
                "SELECT known_chapters, last_read_chapter_num FROM mangas WHERE id = ?",
                (manga_id,),
            ).fetchone()
            if not row:
                return None
    ...
    def update_tracking_preferences(
        ...
        *,
        ...
        with db.get_connection() as conn:
            row = conn.execute(
                "SELECT last_read_source_id, tracking_source_manga_id FROM mangas WHERE id = ?",
                (manga_id,),
            ).fetchone()
            if not row:
                return None
            ...
            conn.execute("""
                ...
                WHERE id = ?
            """, (
                bool(enabled), bool(notifications), bool(auto_download),
                source_manga_id, bool(enabled), manga_id,
            ))
            conn.commit()
        return self.get_manga(manga_id)
    ...
    def get_tracked_mangas(self) -> list[Dict[str, Any]]:
        with db.get_connection() as conn:
            rows = conn.execute(
                "SELECT * FROM mangas WHERE tracking_enabled = 1 ORDER BY title COLLATE NOCASE"
            ).fetchall()
    ...
    def update_tracking_result(
        ...
        *,
        ...
        with db.get_connection() as conn:
            ...
            """, (
                str(source_manga_id or ""), str(source_name or "")[:120],
                max(0, int(checked_at or 0)), str(error or "")[:500], manga_id,
            ))
            conn.commit()
    ...
    def get_library(self, *, include_storage: bool = True) -> Dict[str, Any]:
        # Sık kullanılan bu yol yalnızca SQLite verisini okur. Ağ/dosya onarımları
        # /api/library/maintenance üzerinden arka planda çalıştırılır.
        lib = {"mangas": {}}
        with db.get_connection() as conn:
    ...
    def get_manga_storage_bytes(self, manga: Dict[str, Any]) -> int:
        ...
        # Kapak klasörü bir bölüm klasörünü kapsıyorsa alt yolu ikinci kez sayma.
        unique_roots = []
        for root in sorted(roots, key=len):
            if any(
                os.path.commonpath([root, parent]) == parent
    ...
    def _managed_path_info(path: str) -> tuple[str, str, str] | None:
        ...
        for root, url_prefix in (
            (DOWNLOADS_DIR, "/downloads"),
            (LOCAL_MANGA_DIR, "/local-manga"),
        ):
    ...
    def remove_downloaded_chapter(self, manga_id: str, chapter_id: str) -> bool:
        ...
        if chapter_id in chapters:
            ...
            # Check if there are any remaining chapters
                ...
                if changed:
                    ...
                    if not c_rows_after:
                        if m_row["last_read_chapter"]:
                            ...
                            self.remove_manga(manga_id) # remove manga logic
                            changed = True
            ...
            for manga in cur.fetchall():
                ...
                if manga_id.startswith("anilist_"):
                    if not manga_dict.get("cover_url") or not manga_dict.get("description") or manga_dict.get("status") == "unknown":
                        try:
                            from anilist import get_anilist_metadata
                            ...
                            if search_title:
                                ...
                                if meta:
                                    ...
                                    updates.append("cover_url = ?"); params.append(meta["cover_url"])
                                    updates.append("status = ?"); params.append(meta["status"])
                                    if meta.get("tags"):
                                        updates.append("tags = ?"); params.append(json.dumps(meta["tags"]))
                                    if meta.get("year"):
                                        updates.append("year = ?"); params.append(meta["year"])
                                    ...
                                    if not manga_dict.get("cover_path"):
                                        try:
                                            ...
                                            import io
                                            from migrate_folders import _safe_folder_name, _set_folder_icon
                                            ...
                                            if resp.status_code == 200:
                                                ...
                                                rel_path = os.path.relpath(cover_path, BASE_DIR).replace("\\", "/")
                                                updates.append("cover_path = ?"); params.append(rel_path)
        ...
        with db.get_connection() as conn:
            ...
            for manga in cur.fetchall():
                ...
                try:
                    ...
                    import httpx
                    from PIL import Image
                    from migrate_folders import _safe_folder_name
