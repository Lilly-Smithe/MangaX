...
async def lifespan(app: FastAPI):
    # Startup
    migrate_downloads()   # Eski UUID klasörlerini düzenli isimlere taşı
    start_services()
    ...
    # Shutdown — son yerel yedeği al, sonra scraper'ları ve Chrome'u kapat
...
app = FastAPI(
    title="MangaX API",
    description="Manga Downloader and Reader Backend",
    lifespan=lifespan
)
    ...
    try:
        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            try:
                ...
                # ChromeDriver'ları doğrudan kapat
                if "chromedriver" in name:
                    proc.terminate()
                    killed += 1
                    continue
                
                # Headless Chrome'ları kapat
...
def _free_port(port: int) -> bool:
    ...
    try:
        for conn in psutil.net_connections(kind="inet"):
            # Boş laddr kontrolü (bazen tuple boş dönebiliyor)
            if not conn.laddr:
                continue
            
            # laddr bir tuple değil, namedtuple; port attribute'u olmalı
...
if __name__ == "__main__":
    ...
    # Başlangıçta arkada kalan yetim tarayıcı ve driver süreçlerini temizle
    _close_app_tabs()
